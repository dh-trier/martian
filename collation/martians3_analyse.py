...
def get_difftext(difffile): 
    ...
    with open(difffile, "r") as infile: 
        ...
        difftext = re.split("\n", difftext)
        #print("   number of lines", len(difftext))
        return difftext
...
def create_pairs(line): 
    ...
    """
    pairs = []
    somepairs = re.findall("\[-.*?\-\] {\+.*?\+}", line, re.DOTALL)
