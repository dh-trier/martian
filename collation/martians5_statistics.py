...
def ana_stats(anadata, statistics): 
    ...
    statistics["script-identifiable"] = statistics["capitalization"]+statistics["whitespace"]+statistics["italics"]+statistics["punctuation"]+statistics["hyphenation"]+statistics["numbers"]
    statistics["other edits"] = statistics["condensation"]+statistics["expansion"]+statistics["deletion"]+statistics["insertion"]+statistics["to be confirmed"]
    statistics = pd.DataFrame.from_dict(statistics, orient="index", columns=["values"])
    return statistics
...
def save_pivotdata(anadata, pivotstatisticsfile): 
    ...
    """
    pivotdata = anadata.pivot_table(index="main-type", columns="lev-dist-class", aggfunc='size', fill_value=0)
...
def define_style():
  ...
  value_font_size = 20,
  colors=("DarkSlateGray", "DarkSlateGray", "DarkSlateGray", "DarkSlateGray", "DarkSlateGray", "DarkSlateGray"))
  return mystyle
...
def main(analysisfile, levdistsfile, statisticsfile, pivotstatisticsfile, typesplot1, typesplot2):
    print("-- Running martians5_statistics")
    # (1) Collect data
    statistics = {}
    levdata = read_datafile(levdistsfile)
    statistics = lev_stats(levdata, statistics)
    anadata = read_datafile(analysisfile)
    statistics = ana_stats(anadata, statistics)
    save_data(statistics, statisticsfile)
    pivotdata = save_pivotdata(anadata, pivotstatisticsfile)
    # (2) Visualize data
    selection1 = ["italics", "whitespace", "hyphenation", "capitalization", "punctuation", "numbers"]
    data1 = prepare_simple_data(statistics, selection1)
    make_barchart1(data1, typesplot1)
    data2 = prepare_complex_data(pivotdata)
    make_barchart2(data2, typesplot2)

if __name__ == "__main__":
    main(analysisfile, levdistsfile, statisticsfile, pivotstatisticsfile, typesplot1, typesplot2)
